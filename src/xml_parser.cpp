...
result<storage_data> XmlParser::get_storage(const std::string& file_str)
{
	...
	for (auto author_child = authors_node.child("author"); author_child; author_child = author_child.next_sibling("author"))
	{
		...
		author_list.push_back(std::shared_ptr<Author>(new Author(author_id_int, author_name.value())));
	}
	...
	for (auto book_child = books_node.child("book"); book_child; book_child = book_child.next_sibling("book"))
	{
		...
		if (!(book_author_id && book_id && title))
		{
			continue;
		}
...
result<std::string> XmlParser::add_node(const std::string& nodes_path,
...
result<std::string> XmlParser::add_book(std::shared_ptr<const Book> book)
{
	...
	return std::move(add_node("books", "next_book_id", book_node_filler));
};

result<std::string> XmlParser::add_author(std::shared_ptr<const Author> author)
{
	...
	return std::move(add_node("authors", "next_author_id", author_node_filler));
};

result<std::string> XmlParser::change_node(const std::string& nodes_path,
...
result<std::string> XmlParser::change_book(std::shared_ptr<const Book> book)
{
	...
	return std::move(change_node("books", book_node_filler, find_book_by_id));
};

result<std::string> XmlParser::change_author(std::shared_ptr<const Author> author)
{
	...
	return std::move(change_node("authors", author_node_filler, find_author_by_id));
};

result<std::string> XmlParser::delete_node_by_id(const std::string& nodes_path, int node_id)
{
	pugi::xml_node root = m_doc.document_element();
...
result<std::string> XmlParser::delete_book(int book_id)
{
	return std::move(delete_node_by_id("books", book_id));
};

result<std::string> XmlParser::delete_author(int author_id)
{
	return std::move(delete_node_by_id("authors", author_id));
};
